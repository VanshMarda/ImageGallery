...
function truncateTextToTwoLines({span,text}:ObjectCreated):void{
    // Reset
   span.textContent = text;
    ...
    // If it fits, keep full text
    if (span.scrollHeight <= maxHeight) {
        return;
    }

    let startLen = Math.floor(text.length * 0.6); // keep 60% at start
    let endLen = Math.floor(text.length * 0.3);   // keep 30% at end
    let truncated = text.slice(0, startLen) + "..." + text.slice(text.length - endLen);

    span.textContent = truncated;

    // Adjust until it fits into exactly 2 lines
...
async function loadData():Promise<void> {
    try {
        ...
        data.forEach((element:ImageData,index:number) => {
            const unOrderedList = document.querySelector<HTMLUListElement>(".image-list") 
            ...
            requestAnimationFrame(() => {
                truncateTextToTwoLines({span:newSpan, text:element.text});
            });
